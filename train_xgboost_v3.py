"""
AD vs CN EEG Classification using XGBoost — v3 (Enhanced Features + Optuna + Test Inference)
==============================================================================================
Improvements over v2:
  ...
  4. Phase 7: Inference on unlabeled test subjects → submission CSV
...
def extract_features_v3(segment, sfreq=SFREQ):
    ...
    for ch in range(n_channels):
        ...
        # Statistical moments
        features.extend([np.mean(signal), np.std(signal),
    ...
    for ch1, ch2 in itertools.combinations(range(n_channels), 2):
        ...
        for band_name, (fmin, fmax) in BANDS.items():
            ...
            features.append(float(Cxy[idx].mean()) if idx.any() else 0.0)
...
def _clean(X):
    """Replace NaN/Inf with 0 (can arise from flat channels in entropy)."""
    bad = ~np.isfinite(X)
    if bad.any():
...
def tune_hyperparams(X, y, groups, n_trials=OPTUNA_TRIALS):
    ...
    Optimises mean snippet-level AUC-ROC. Tuned params passed to all
    subsequent phases (LOSO, final model, overfitting analysis).
    """
    print("\n" + "=" * 70)
...
def _make_model(scale_pos_weight=1.0, **kwargs):
    """Return an XGBClassifier. kwargs override the v2 conservative defaults."""
    defaults = dict(
        max_depth=4, learning_rate=0.03, min_child_weight=7,
        subsample=0.6, colsample_bytree=0.6,
        reg_alpha=0.1, reg_lambda=5, gamma=0.5,
    )
...
def loso_cv(X, y, groups, feature_names, best_params=None):
    ...
    print("=" * 70)
    print("Using Optuna-tuned hyperparameters." if best_params
          else "Using default (v2) hyperparameters.")
    ...
    for fold_i, (train_idx, test_idx) in enumerate(logo.split(X, y, groups)):
        ...
        X_tf = X[train_idx]
        y_tf = y[train_idx]
        g_tf = groups[train_idx]

        # Fresh internal early-stopping split (never touches the test subject)
        rng        = np.random.RandomState(RANDOM_STATE + fold_i)
        train_sids = np.unique(g_tf)
        rng.shuffle(train_sids)
        n_val      = max(1, int(len(train_sids) * 0.15))
        val_sids   = set(train_sids[:n_val])
        inner_sids = set(train_sids[n_val:])

        it_mask = np.array([g in inner_sids for g in g_tf])
        iv_mask = np.array([g in val_sids   for g in g_tf])

        X_it, y_it = X_tf[it_mask], y_tf[it_mask]
        X_iv, y_iv = X_tf[iv_mask], y_tf[iv_mask]

        spw   = (y_it == 0).sum() / max((y_it == 1).sum(), 1)
    ...
    ad_res = [r for r in subject_results if r["true"] == "AD"]
    cn_res = [r for r in subject_results if r["true"] == "CN"]

    print(f"\n{'='*70}")
    print(f"LOSO-CV RESULTS (TRUE ACCURACY)")
    print(f"{'='*70}")
    print(f"Subject-level accuracy: {n_correct}/{n_subjects} = {subj_acc:.4f}")
    print(f"  AD recall: {sum(r['correct'] for r in ad_res)}/{len(ad_res)}"
          f" = {sum(r['correct'] for r in ad_res)/len(ad_res):.4f}")
    print(f"  CN recall: {sum(r['correct'] for r in cn_res)}/{len(cn_res)}"
          f" = {sum(r['correct'] for r in cn_res)/len(cn_res):.4f}")
    ...
    pd.DataFrame(subject_results).to_csv(
        os.path.join(OUTPUT_DIR, "loso_subject_results_v3.csv"), index=False)
    print("Saved: loso_subject_results_v3.csv")
    ...
    plt.title(f"LOSO-CV Confusion Matrix (Subject-Level)\n"
              f"Accuracy: {n_correct}/{n_subjects} = {subj_acc:.1%}")
    plt.tight_layout()
    ...
    ax.bar(range(len(sids)), probs_list, color=colors,
...
def train_final_model(X, y, groups, feature_names, best_params=None):
    """Train on ALL labeled data using tuned hyperparameters."""
    print("\n" + "=" * 70)
...
def overfitting_analysis(X, y, groups, best_params=None):
    ...
    for fold_i, (tr_idx, va_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        print(f"  Fold {fold_i+1}: train={tr_acc:.4f}  val={va_acc:.4f}  "
              ...
              f"(train={len(np.unique(groups[tr_idx]))} subj, "
              f"val={len(np.unique(groups[va_idx]))} subj, "
...
# ─────────────────────────────────────────────────────────────────────────────
# PHASE 7 — TEST SET INFERENCE
# ─────────────────────────────────────────────────────────────────────────────
def predict_test_set(final_model, feature_names):
    """
    Run the final model on all unlabeled subjects in training/test/.

    For each subject:
      1. Load .npy (19, time)
      2. Segment into 30s windows (same params as training)
      3. Extract 1330 features per segment
      4. Soft vote: mean P(AD) across segments → threshold at 0.5

    Outputs:
      output/test_predictions.csv — subject_id, predicted_label, prob_AD, n_snippets
    """
    print("\n" + "=" * 70)
    print("PHASE 7: Test set inference")
    print("=" * 70)
    ...
    print(f"Found {len(test_files)} test subjects in {TEST_DIR}")

    results = []
    for path in test_files:
        sid = int(os.path.splitext(os.path.basename(path))[0])
        eeg = np.load(path, allow_pickle=True)

        if eeg.shape[0] != 19:
            print(f"  WARNING: subject {sid} has {eeg.shape[0]} channels, skipping")
            continue

        # Segment
        segs = []
        for start in range(0, eeg.shape[1] - WINDOW_SAMPLES + 1, STEP_SAMPLES):
            segs.append(eeg[:, start:start + WINDOW_SAMPLES])

        if not segs:
            print(f"  WARNING: subject {sid} recording too short, skipping")
            continue

        # Extract features
        X_subj = np.array([extract_features_v3(s)[0] for s in segs])
        X_subj = np.nan_to_num(X_subj, nan=0.0, posinf=0.0, neginf=0.0)

        # Soft vote
        probs    = final_model.predict_proba(X_subj)[:, 1]
        avg_prob = float(np.mean(probs))
        pred     = "AD" if avg_prob >= 0.5 else "CN"
        ...
        print(f"  Subject {sid:>3d}: pred={pred}  P(AD)={avg_prob:.3f}"
              f"  ({len(segs)} snippets)")
    ...
    ad_count = (df["predicted_label"] == "AD").sum()
    cn_count = (df["predicted_label"] == "CN").sum()
    print(f"\nTest predictions summary: {ad_count} AD, {cn_count} CN "
          f"out of {len(df)} subjects")
...
def main():
    ...
    print(f"Features: 22×19 + 3×19 (nonlinear) + 171×5 (coherence) = 1330")
    print()

    # Phase 1: Load labeled data
    ...
    # Phase 4: LOSO-CV (true accuracy estimate)
    subj_acc, subject_results, _ = loso_cv(
        X, seg_labels, seg_subjects, feature_names, best_params=best_params
    )

    # Phase 5: Final model on all labeled data
    final_model = train_final_model(
        X, seg_labels, seg_subjects, feature_names, best_params=best_params
    )

    # Phase 6: Overfitting analysis
    avg_gap = overfitting_analysis(
        X, seg_labels, seg_subjects, best_params=best_params
    )

    # Phase 7: Inference on unlabeled test subjects
    test_df = predict_test_set(final_model, feature_names)
    ...
    if test_df is not None:
        print(f"\nTest set predictions ({len(test_df)} subjects):")
